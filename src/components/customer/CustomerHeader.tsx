...
export function CustomerHeader() {
    ...
    const parentCategories = categories.filter(c => !c.parentId && c.name.toLowerCase() !== 'home' && c.name.toLowerCase() !== 'beauty' && c.name.toLowerCase() !== 'studio');
    ...
    return (
        <header className="sticky top-0 z-50 w-full border-b bg-background shadow-sm">
            <div className="flex h-20 items-center px-6 lg:px-12">
                            ...
                            {parentCategories.map(category => {
                                ...
                                if (!hasSubcategories) {
                                    <NavigationMenuItem key={category.id}>
                                        <NavigationMenuLink asChild>
                                            <Link href={`/category/${category.name.toLowerCase()}`} className={cn(navigationMenuTriggerStyle(), "font-bold tracking-wider text-sm bg-transparent")}>
                                                {category.name.toUpperCase()}
                                            </Link>
                                        </NavigationMenuLink>
                                    </NavigationMenuItem>
